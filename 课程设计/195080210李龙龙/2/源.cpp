...
typedef struct Node {
	int data;  //存放数据
	struct Node* next;  //存放下一个数据的位置的指针
}NodeList;
...
NodeList* Input() {
	...
	while ((c = getchar()) != '\n') { // 输入整数，按字符接收数字
		if (c >= '0' && c <= '9') { // 若为数字则存入
			...
			q->num = int(c - '0'); // 利用ASCII码差值存入一位整数，将获取的字符型转化为整形，存入链表
			q->np = p; // 建立指针
			p = q;
		}
	}
...
int main() {
	...
	int f;
	char c;
	NodeList* Input(), * Add(NodeList * p, NodeList * q), * InsetAfter(), * Sub(NodeList * p, NodeList * q);
